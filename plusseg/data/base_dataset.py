...
class BaseDataset(data.Dataset):
    ...
    def val_sync_transform(self, img, mask):
        ...
        mask = img.resize((ow, oh), Image.NEAREST)

        # center crop
        w, h = img.size

        x1 = int(round(w - out_size) / 2.0)
        y1 = int(round(h - out_size) / 2.0) 
        img = img.crop((x1, y1, x1+out_size, y1+out_size))
        mask = mask.crop((x1, y1, x1+out_size, y1+out_size))
    ...
    def sync_transform(self, img, mask):
        ...
        # Padding when the image size small than the crop_size
        if short_size < crop_size:
            pad_h = crop_size - oh if oh < crop_size else 0
            pad_w = crop_size - ow if ow < crop_size else 0
...
def test_batchify_fn(data):
    error_msg = "Batch Must Contains Tensors, tuple or lists, found {}"
